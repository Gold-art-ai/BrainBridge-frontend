...
export default function InboxClient() {
  ...
  useEffect(() => {
    ...
    if (isNewChat && projectTag) {
      ...
      setChats(prev => {
        ...
        return [
          { id: newChatId, name: recipient, project: projectTag, lastMsg: "Drafting..." },
          ...prev
        ];
      });
  ...
  const sendMessage = (e) => {
    ...
    setChats(prev =>
      prev.map(c =>
        c.id === activeChatId ? { ...c, lastMsg: inputText } : c
      )
    );
  ...
  return (
    <div className="flex h-[calc(100vh-120px)] bg-white rounded-[2.5rem] border border-gray-100 overflow-hidden shadow-sm">
      ...
      <div className="w-80 border-r border-gray-50 flex flex-col bg-white">
        <div className="p-6">
          <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-[#08075C] mb-6">
            Active Nodes
          </h2>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="overflow-hidden">
                  ...
                  <p className="text-[9px] opacity-60 truncate font-medium uppercase tracking-tight">
                    {chat.project ? `Proj: ${chat.project}` : chat.lastMsg}
                  </p>
                </div>
              </div>
            </div>
      ...
      <div className="flex-1 flex flex-col bg-gray-50/30">
        <div className="p-6 bg-white border-b border-gray-50 flex justify-between items-center">
          <h3 className="text-sm font-black text-[#08075C]">
            {activeChat.name}
          </h3>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-6">
          {messages
            .filter(
              m => !m.project || m.project === activeChat.project
            )
            .map((msg) => (
              <div
                key={msg.id}
              ...
              >
                <div
                  className={`max-w-[70%] p-4 rounded-2xl text-xs leading-relaxed shadow-sm ${
                    msg.sender === "You"
                      ? "bg-[#08075C] text-white rounded-tr-none"
                      : "bg-white text-[#08075C] border border-gray-100 rounded-tl-none font-semibold"
                  }`}
                >
                  {msg.text}
                </div>

                <span className="text-[8px] font-black uppercase text-gray-300 mt-2 tracking-widest">
                  {msg.sender}
                </span>
              </div>
            ))}
        ...
        <div className="p-6 bg-white border-t border-gray-50">
          <form onSubmit={sendMessage} className="relative">
            ...
            <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
              ...
              <button
                type="submit"
                className="bg-[#3A38DE] text-white px-5 py-2.5 rounded-xl hover:bg-[#08075C] transition-all flex items-center gap-2"
              >
